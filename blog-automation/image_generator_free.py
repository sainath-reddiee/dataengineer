...
class ImageGeneratorFree:
    ...
    def _enhance_prompt(self, base_prompt: str) -> str:
        """Enhance prompt for a consistent hand-drawn watercolor style."""
        if not base_prompt:
            base_prompt = "technical illustration"
            
        style_additions = (
            "hand-drawn watercolor illustration, loose watercolor brush technique, "
            "cool blues, warm ambers, earthy greens, visible paper texture, "
            "technical sketch, no corporate aesthetics, notebook style, data engineering theme"
        )
