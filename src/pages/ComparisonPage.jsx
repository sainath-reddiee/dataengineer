    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                setComparison(data);
                setDerivedData({ meta, canonical, relatedComparisons, crossLinkedTerms });
                ...
                <div className="bg-slate-800/80 border-b border-slate-700/50">
                    <div className="max-w-6xl mx-auto px-4 py-3">
                        <nav className="flex items-center gap-2 text-sm">
                            ...
                            <Link to="/compare" className="text-gray-300 hover:text-white transition-colors">Comparisons</Link>
                            <ChevronRight className="w-4 h-4 text-gray-500" />
                            <span className="text-blue-400 font-medium">{comparison.toolA} vs {comparison.toolB}</span>
                        </nav>
                    </div>
                </div>
